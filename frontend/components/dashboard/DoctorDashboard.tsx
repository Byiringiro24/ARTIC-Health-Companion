        ...
        <div style={{ flex:1,overflowY:"auto",padding:18 }}>
          ...
          {section==="settings" && (
            <div style={{ display:"grid",gap:16,maxWidth:640 }}>
              <div><div style={{ fontWeight:700,fontSize:15,color:"#0f172a" }}>Settings & Security</div>
              <div style={{ fontSize:11,color:"#94a3b8" }}>Password, profile, preferences</div></div>

              {/* OTP Password Change Card */}
              <div style={{ background:"white",borderRadius:14,border:"2px solid #0891b2",overflow:"hidden" }}>
                <div style={{ padding:"14px 18px",borderBottom:"1px solid #f1f5f9",background:"linear-gradient(135deg,#f0f9ff,#e0f2fe)",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                  <div>
                    <div style={{ fontWeight:800,fontSize:13,color:"#0f172a",display:"flex",alignItems:"center",gap:6 }}><Key size={14} style={{ color:"#0891b2" }}/>Change Password — 3-Step Secure Verification</div>
                    <div style={{ fontSize:11,color:"#64748b",marginTop:2 }}>Current password → OTP by email → New password</div>
                  </div>
                ...
                <div style={{ padding:"18px 20px" }}>
                      ...
                      return (
                        <div key={s} style={{ display:"flex",alignItems:"center",gap:5,flex:1 }}>
                          <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:3,flex:1 }}>
                            ...
                            <div style={{ fontSize:9,color:active?"#0891b2":done?"#059669":"#94a3b8",fontWeight:active||done?600:400,textAlign:"center" }}>{labels[i]}</div>
                          </div>
                  ...
                  {otpStep==="current" && (
                    <div style={{ maxWidth:400 }}>
                      <div style={{ background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:8,padding:"9px 12px",marginBottom:14,fontSize:12,color:"#0369a1" }}>
                        🔐 Enter your current password to receive an OTP at <strong>{user?.email||"your email"}</strong>.
                      </div>
                      <label style={{ fontSize:12,fontWeight:600,color:"#374151",display:"block",marginBottom:4 }}>Current Password</label>
                      <div style={{ position:"relative",marginBottom:14 }}>
                        <input type={showPw?"text":"password"} value={pwCurrent} onChange={e=>setPwCurrent(e.target.value)} onKeyDown={e=>e.key==="Enter"&&stepRequestOTP()} placeholder="Enter current password"
                          style={{ width:"100%",padding:"9px 36px 9px 11px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:12,outline:"none",boxSizing:"border-box",color:"#0f172a" }}/>
                        <button type="button" onClick={()=>setShowPw(!showPw)} style={{ position:"absolute",right:9,top:"50%",transform:"translateY(-50%)",border:"none",background:"none",cursor:"pointer",color:"#64748b",display:"flex" }}>
                          {showPw?<EyeOff size={15}/>:<Eye size={15}/>}
                        </button>
                      </div>
                      <button onClick={stepRequestOTP} disabled={pwLoading||!pwCurrent}
                        style={{ display:"flex",alignItems:"center",gap:6,padding:"10px 20px",background:pwLoading||!pwCurrent?"#e2e8f0":"linear-gradient(135deg,#0891b2,#7c3aed)",color:pwLoading||!pwCurrent?"#94a3b8":"white",borderRadius:9,border:"none",cursor:pwLoading||!pwCurrent?"not-allowed":"pointer",fontSize:12,fontWeight:700 }}>
                        <Mail size={13}/>{pwLoading?"Verifying…":"Verify & Send OTP"}
                      </button>
                    </div>
                  )}
                  ...
                  {otpStep==="otp" && (
                    <div style={{ maxWidth:400 }}>
                      <div style={{ background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:12,color:"#065f46" }}>
                        ✅ OTP sent! {otpHint}. Valid for 10 minutes.
                      </div>
                      <label style={{ fontSize:12,fontWeight:600,color:"#374151",display:"block",marginBottom:4 }}>6-Digit OTP Code</label>
                      <input type="text" inputMode="numeric" maxLength={6} value={pwOtp} onChange={e=>setPwOtp(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="000000"
                        style={{ width:190,padding:"11px 14px",borderRadius:9,border:`2px solid ${pwOtp.length===6?"#059669":"#e2e8f0"}`,fontSize:22,fontWeight:800,fontFamily:"monospace",letterSpacing:"0.25em",textAlign:"center",outline:"none",color:"#0f172a",boxSizing:"border-box",marginBottom:14 }}/>
                      <div style={{ display:"flex",gap:8 }}>
                        <button onClick={resetOTP} style={{ padding:"8px 14px",border:"1px solid #e2e8f0",background:"white",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:600,color:"#374151" }}>← Back</button>
                        <button onClick={stepConfirmOTP} disabled={pwOtp.length!==6}
                          style={{ display:"flex",alignItems:"center",gap:6,padding:"9px 20px",background:pwOtp.length!==6?"#e2e8f0":"linear-gradient(135deg,#059669,#0891b2)",color:pwOtp.length!==6?"#94a3b8":"white",borderRadius:9,border:"none",cursor:pwOtp.length!==6?"not-allowed":"pointer",fontSize:12,fontWeight:700 }}>
                  ...
                  {otpStep==="newpw" && (
                    <div style={{ maxWidth:400 }}>
                      <div style={{ background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:"9px 12px",marginBottom:14,fontSize:12,color:"#065f46" }}>✅ Identity confirmed. Set your new password.</div>
                        ...
                        <div>
                          ...
                          <div style={{ position:"relative" }}>
                            <input type={showPw?"text":"password"} value={pwNew} onChange={e=>setPwNew(e.target.value)} placeholder="Min 8 characters"
                              style={{ width:"100%",padding:"9px 36px 9px 11px",borderRadius:8,border:"1px solid #e2e8f0",fontSize:12,outline:"none",boxSizing:"border-box",color:"#0f172a" }}/>
                            <button type="button" onClick={()=>setShowPw(!showPw)} style={{ position:"absolute",right:9,top:"50%",transform:"translateY(-50%)",border:"none",background:"none",cursor:"pointer",color:"#64748b",display:"flex" }}>{showPw?<EyeOff size={15}/>:<Eye size={15}/>}</button>
                          </div>
                          {pwNew && <div style={{ marginTop:5 }}>
                            <div style={{ display:"flex",justifyContent:"space-between",fontSize:10,marginBottom:2 }}><span style={{ color:"#64748b" }}>Strength</span><span style={{ color:pwStrC,fontWeight:700 }}>{pwStr}</span></div>
                            <div style={{ height:3,background:"#f1f5f9",borderRadius:4 }}><div style={{ height:"100%",width:`${pwStrP}%`,background:pwStrC,borderRadius:4,transition:"width 0.3s" }}/></div>
                          </div>}
                        </div>
                        <div>
                          ...
                          <input type={showPw?"text":"password"} value={pwConfirm} onChange={e=>setPwConfirm(e.target.value)} placeholder="Re-enter new password"
                            style={{ width:"100%",padding:"9px 11px",borderRadius:8,border:`1px solid ${pwConfirm&&pwConfirm!==pwNew?"#fca5a5":pwConfirm&&pwConfirm===pwNew&&pwNew.length>=8?"#86efac":"#e2e8f0"}`,fontSize:12,outline:"none",boxSizing:"border-box",color:"#0f172a" }}/>
                          {pwConfirm&&pwConfirm!==pwNew && <div style={{ fontSize:10,color:"#dc2626",marginTop:2 }}>✗ Passwords don&apos;t match</div>}
                          {pwConfirm&&pwConfirm===pwNew&&pwNew.length>=8 && <div style={{ fontSize:10,color:"#059669",marginTop:2 }}>✓ Passwords match</div>}
                        </div>
                      </div>
                      <div style={{ display:"flex",gap:8 }}>
                        <button onClick={()=>setOtpStep("otp")} style={{ padding:"8px 14px",border:"1px solid #e2e8f0",background:"white",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:600,color:"#374151" }}>← Back</button>
                        <button onClick={stepChangePassword} disabled={pwLoading||!pwNew||pwNew!==pwConfirm||pwNew.length<8}
                          style={{ display:"flex",alignItems:"center",gap:6,padding:"9px 20px",background:pwLoading||!pwNew||pwNew!==pwConfirm||pwNew.length<8?"#e2e8f0":"linear-gradient(135deg,#059669,#0891b2)",color:pwLoading||!pwNew||pwNew!==pwConfirm||pwNew.length<8?"#94a3b8":"white",borderRadius:9,border:"none",cursor:"pointer",fontSize:12,fontWeight:700 }}>
                          <Key size={13}/>{pwLoading?"Changing…":"Change Password"}
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {otpStep==="done" && (
                    <div style={{ textAlign:"center",padding:"16px 0" }}>
                      ...
                      <div style={{ fontSize:15,fontWeight:800,color:"#059669" }}>Password Changed!</div>
                      <div style={{ fontSize:12,color:"#64748b",marginTop:4 }}>Logging you out in a moment…</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Profile */}
              <div style={{ background:"white",borderRadius:12,border:"1px solid #e2e8f0",padding:"16px 18px" }}>
                <div style={{ fontWeight:700,fontSize:12,color:"#0f172a",marginBottom:12 }}>👤 Profile Information</div>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
                  {[
                    ...
                    { l:"Role",          v:"Doctor" },
                    { l:"Job Title",     v:user?.jobTitle||"Doctor" },
                  ].map(f=>(
              ...
              {/* Notification prefs */}
              <div style={{ background:"white",borderRadius:12,border:"1px solid #e2e8f0",padding:"16px 18px" }}>
                <div style={{ fontWeight:700,fontSize:12,color:"#0f172a",marginBottom:10 }}>🔔 Notification Preferences</div>
                {[
                  { l:"Critical Lab Results",        sub:"Immediate alert for critical/panic values", on:true },
                  { l:"New Appointment Requests",    sub:"When patients book or request appointments", on:true },
                  { l:"Prescription Dispensed",      sub:"When pharmacy dispenses your prescriptions", on:false },
                  { l:"Daily Patient Summary",        sub:"Morning summary of today's scheduled patients",on:true },
                ].map((n,i)=>(
                  <div key={i} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 10px",background:"#f8fafc",borderRadius:8,marginBottom:6 }}>
                    <div><div style={{ fontSize:12,fontWeight:600,color:"#0f172a" }}>{n.l}</div><div style={{ fontSize:10,color:"#94a3b8" }}>{n.sub}</div></div>
                    <div style={{ width:38,height:20,borderRadius:10,background:n.on?"#0891b2":"#d1d5db",cursor:"pointer",position:"relative" }} onClick={()=>showToast(`${n.l} ${n.on?"disabled":"enabled"}`)}>
                      <div style={{ width:16,height:16,borderRadius:"50%",background:"white",position:"absolute",top:2,transition:"left 0.2s",left:n.on?20:2 }}/>
                    </div>
                  </div>
                ))}
              </div>
