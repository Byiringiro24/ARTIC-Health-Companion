...
export async function processAIQuery(query, userId) {
  ...
  if (process.env.OPENAI_API_KEY) {
    try {
      // Gather system context (aggregated — no clinical PII)
      const db = getDb();
      ...
      const systemContext = `You are the ARTIC Health Companion AI, integrated into a hospital management system serving ${hospCount?.n||0} hospitals with ${userCount?.n||0} active healthcare users in Rwanda. You have deep knowledge of:
- Rwanda Ministry of Health (MOH) clinical protocols and guidelines (2024)
...
- Rwanda Data Protection Law (HIPAA-equivalent privacy rules)

The system currently has ${featCount?.n||0} active features. Answer questions about clinical guidance, health education, medication information, Rwanda MOH protocols, and system management. Always recommend consultation with qualified medical professionals for individual patient decisions. Responses should be concise and actionable.`;
      ...
      if (res.ok) {
        ...
        if (response) {
          response += "\n\n⚕️ Powered by ARTIC AI with Rwanda MOH protocols. Verify with qualified medical professionals.";
        }
      }
    } catch (e) { console.warn("OpenAI call failed, using local KB:", e.message); }
  }
  ...
  if (!response) {
    ...
    response = `Regarding your query about "${query}":\n\n${matched}\n\n⚕️ This is informational guidance aligned with Rwanda MOH protocols. All clinical decisions must be made by qualified medical professionals licensed in Rwanda.`;
  }
  ...
  return { query, response, timestamp: new Date().toISOString(), source: process.env.OPENAI_API_KEY ? "openai" : "local-kb" };
}
